    ...
    public class UserStepDefinitions
    {
        ...
        public void WhenIRequestToUpdateAnAccount()
        {
            try
            {
                ...
                _client.SetBearerToken(response.Data!.Token);
                var request = new HttpRequestMessage(HttpMethod.Put, "api/Auth/updateUser?id=4")
                {
                    Content = content
                };
        ...
        public void WhenIClickTheLink()
        {
            try
            {
                ...
                var db = serviceContainer.ServiceProvider.GetRequiredService<UserContext>();
                var userList = db.Users.ToList();
                var user = string.IsNullOrEmpty(forgotPasswordContent.Email) ? userList.FirstOrDefault(x => x.UserName.Equals(forgotPasswordContent.UserName)) : userList.FirstOrDefault(x => x.Email.Equals(forgotPasswordContent.Email));
